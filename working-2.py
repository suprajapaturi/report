    ...
    def generate_report(self, output_path: str = "output_pdf.pdf") -> bool:
        ...
        try:
            ...
            if form_fields:
                ...
                # Add page numbering - Get total page count
                total_pages = len(writer.pages)
                print(f"DEBUG: Total pages for numbering: {total_pages}")
                
                # Fill the form fields on all pages
                print(f"DEBUG: Updating {len(form_data)} form fields across all pages")
                
                # Update form fields on each page and add page numbering
                for page_idx, page in enumerate(writer.pages):
                    current_page_data = form_data.copy()
                    
                    # Add page-specific numbering
                    page_number = page_idx + 1
                    
                    # Add page numbering and report identification on pages 2-6
                    if page_number >= 2 and page_number <= 6:
                        # For pages with "Page X of Y" field (like page 2)
                        if f'Page {page_number} of' in form_fields:
                            current_page_data[f'Page {page_number} of'] = str(total_pages)
                            print(f"DEBUG: Setting 'Page {page_number} of' = {total_pages}")
                        
                        # For pages 3-6: Add report identification using TextField1[0]
                        if page_number >= 3:
                            # Use TextField1[0] for report identification following sample format: "1234 Main Street Denton Texas 76201 - 09/30/2021"
                            if 'TextField1[0]' in form_fields:
                                current_page_data['TextField1[0]'] = f'{property_address} - {inspection_date}'
                                print(f"DEBUG: Setting TextField1[0] on page {page_number} = '{property_address} - {inspection_date}'")
                            
                            # Use TextField2[0] for just the total page count "6"
                            if 'TextField2[0]' in form_fields:
                                current_page_data['TextField2[0]'] = str(total_pages)
                                print(f"DEBUG: Setting TextField2[0] on page {page_number} = '{total_pages}'")
                    
                    # Update this page's form fields
                    writer.update_page_form_field_values(page, current_page_data)
